...
import { createFlowerShop, searchApprovedUsers, type SearchResult } from "@/lib/api"
...
interface AddFlowerShopModalProps {
  open: boolean
  onClose: () => void
  onSuccess: () => void
}

export function AddFlowerShopModal({ open, onClose, onSuccess }: AddFlowerShopModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!name.trim()) {
      setError("Please enter flower shop name")
      return
    }
    if (!address.trim()) {
      setError("Please enter flower shop address")
      return
    }
    setSubmitting(true)
    setError(null)
    try {
      await createFlowerShop({
      ...
      setError(e instanceof Error ? e.message : "Failed to create flower shop")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <Dialog open={open} onClose={handleClose} title="Add Flower Shop" className="max-w-xl">
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        >
          {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Add Flower Shop"}
        </Button>
      </div>
    </Dialog>
  )
